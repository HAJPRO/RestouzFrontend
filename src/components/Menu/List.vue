<template>
  <ion-page class="bg-slate-50 dark:bg-[#020617]">
    ...
    <ion-content :fullscreen="true" class="ion-padding-bottom">
          ...
          >
            <div class="relative w-24 h-24 flex-none overflow-hidden rounded-[22px] bg-slate-100 dark:bg-slate-800">
              ...
              <button 
                ...
                @click="store_menu.removeFromCart(food._id)" 
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center">
                  <div v-if="getItemCount(food._id) > 0" class="flex items-center bg-slate-100 dark:bg-slate-800 rounded-[14px] p-1 gap-1 border border-slate-200 dark:border-white/5">
                    <button @click="handleUpdateQty(food._id, -1)" class="w-8 h-8 rounded-lg bg-white dark:bg-slate-700 text-slate-600 dark:text-white flex items-center justify-center active:scale-75">
                      <ion-icon :icon="removeOutline" />
                    </button>
                    <span class="text-xs font-black min-w-[24px] text-center dark:text-white">{{ getItemCount(food._id) }}</span>
                    <button @click="handleUpdateQty(food._id, 1)" class="w-8 h-8 rounded-lg bg-indigo-600 text-white flex items-center justify-center active:scale-75">
      ...
      <div v-if="store_menu.totalItemsCount > 0" class="fixed bottom-24 left-4 right-4 z-40 animate-slide-up cursor-pointer">
        <div class="bg-white dark:bg-slate-900 rounded-[32px] shadow-2xl border border-slate-100 dark:border-white/10 overflow-hidden" @click="store_menu.CardModalAction()">
          ...
          <div class="p-4 flex items-center justify-between gap-3">
            ...
            <Button size="sm" leftIcon="fas fa-shopping-basket" class="!bg-indigo-600 !rounded-2xl">
...
const filteredFoods = computed(() => {
  ...
  const activeCatKey = categories.find(c => c.id === activeCategory.value)?.key || 'all';
...
const getItemCount = (id) => {
  const item = cartItems.value.find(i => i._id === id);
  return item ? item.quantity : 0;
};

const handleAddToCart = async (food) => {
  store_menu.addToCart({
    id: food._id, // Store kutayotgan formatga moslash
    _id: food._id,
    name: food.name,
    price: food.price,
    image: food.image

          ...
          >
            ...
            <div class="flex justify-between items-start mb-5">
              ...
              <div :class="['w-10 h-10 rounded-2xl flex items-center justify-center shadow-inner', getStatusIconBg(table.status)]">
                <ion-icon :icon="table.status === '0' ? addOutline : cartOutline" class="text-lg" />
              </div>
            </div>

            <div class="min-h-[55px] flex flex-col justify-end">
              <div v-if="table.status !== '0'" class="space-y-2">
                ...
                <div class="text-[16px] font-black text-indigo-600 dark:text-indigo-400">
                  {{table.total? table.total.toLocaleString() : '0'}} <small class="text-[9px] uppercase opacity-60">uzs</small>
                </div>
              </div>
...
const statusFilters = [
  ...
  { id: '0', label: 'Bo\'shlar', icon: 'fas fa-check-circle' },
...
const filteredTables = computed(() => {
  return tabels.value.filter(t => {
    ...
    const matchSearch = t.number.toLowerCase().includes(searchQuery.value.toLowerCase());
...
const getStatusLineColor = (s) => {
  const colors = { 0: 'bg-emerald-500', occupied: 'bg-rose-500', waiting: 'bg-amber-500' };
  return colors[s] || 'bg-slate-200';
};

const getStatusIconBg = (s) => {
  const bgs = {
    0: 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10',

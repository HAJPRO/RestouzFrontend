<template>
  <ion-page class="bg-slate-50 dark:bg-[#020617]">
    ...
    <ion-content :fullscreen="true" class="ion-padding">
      <GlobalRefresher @refresh="refreshCustomers" />
      ...
      <div v-else class="max-w-full mx-auto pb-10 p-4">
          ...
          >
            <div class="p-5">
                    ...
                    <div>
                      ...
                      <div class="flex items-center gap-2 mt-1">
                        <span 
                          ...
                          :class="getCategoryClass(customer.category)"
                        >
                          {{ customer.category || 'Yangi' }}
                        </span>
                        <span class="text-[11px] font-bold text-slate-400">ID: {{ customer._id?.slice(-5) }}</span>
                      </div>
                    </div>
              ...
              <div class="mt-4 grid grid-cols-2 gap-4 py-3 border-y border-slate-50 dark:border-white/5">
                <div class="flex items-start gap-2">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-[11px] text-slate-600 dark:text-slate-300 truncate mt-1">{{ customer.address || 'Kiritilmagan' }}</p>
                  </div>
                </div>
                <div class="flex items-start gap-2">
                  <div class="w-7 h-7 rounded-lg bg-slate-50 dark:bg-white/5 flex items-center justify-center text-slate-400">
                  ...
                  <div>
                    <p class="text-[9px] text-slate-400 font-bold uppercase leading-none">Buyurtmalar</p>
                    <p class="text-[11px] text-indigo-600 font-black mt-1">{{ customer.orderCount || 0 }} marta</p>
                  </div>
                </div>
              </div>
              ...
              <div class="flex gap-2 mt-4">
                <button 
                  @click.stop="callCustomer(customer.phone)"
                ...
                <button 
                  @click.stop="openTelegram(customer.phone)"
        ...
        <EmptyState v-else title="Mijoz topilmadi" />
      </div>
    </ion-content>
  </ion-page>
</template>
...
const getCategoryClass = (cat) => {
  switch(cat) {
    case 'vip': return 'bg-amber-100 text-amber-600 dark:bg-amber-500/20';
    case 'regular': return 'bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20';
...
const refreshCustomers = async (e) => {

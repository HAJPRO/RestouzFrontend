<template>
      ...
      <div class="bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl pt-safe border-b border-slate-100 dark:border-white/5">
        ...
        <div class="max-w-full mx-auto px-4 h-16 flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <h1 class="text-[17px] font-black text-slate-900 dark:text-white tracking-tight">Ruxsatlar</h1>
          </div>
  ...
  <div v-else-if="filteredPermissions.length > 0" class="space-y-4">
    <div 
    ...
    >
      <div class="flex items-start justify-between mb-4">
        <div class="flex items-center gap-4">
          <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center text-white shadow-lg shadow-indigo-500/20">
            <i :class="getPermissionIcon(permission.name)" class="text-xl"></i>
          </div>
          ...
          <div class="flex flex-col">
            ...
            <p class="text-[12px] text-slate-400 font-medium line-clamp-1 italic">
              {{ permission.description || 'Tizimdagi maxsus huquq darajasi' }}
            </p>
          </div>
        </div>

        <button @click="editPermission(permission)" class="w-10 h-10 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-400 flex items-center justify-center active:bg-indigo-600 active:text-white transition-all">
      ...
      <div class="flex items-center justify-between mt-4 pt-4 border-t border-slate-50 dark:border-white/5">
        <div class="flex items-center gap-6">
          <div class="flex flex-col">
            <span class="text-[9px] text-slate-400 font-black uppercase mb-0.5 tracking-wider">Ruxsatlar</span>
...
const addUser = async () => {
  ...
  console.log("Permission qo'shish");
};
